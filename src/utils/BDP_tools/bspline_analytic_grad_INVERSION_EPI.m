...
function [X_error, X_error_grad] = bspline_analytic_grad_INVERSION_EPI( ...
                                                    X, Osize1, O_grid, spacing, rigid_scl, ...
                                                    Iepi, Ist, res, mask_Iepi, mask_Iepi_less_csf, mask_Ist, opt)                                                 

% mask_Iepi_less_csf - Mask using which INVERSION mapping is computed

% Check/set input options
defaultoptions = struct(...
   'penalty', [0.01 0.45 0.46 0.45], ...
   'grad_step_rigid', 0.01, ...
   'debug', false, ...
...
% Apply rigid transform to static image
mode = 1; % linear interpolation and outside pixels set to zero
par = rigid_param.*rigid_scl;
M = par2affineMat(par(1:3),par(4:6));
Ist_rigid = demon_registration.affine_transform_3d_double(double(Ist), double(M), double(mode));
mask_Ist_rigid = demon_registration.affine_transform_3d_double(double(mask_Ist),double(M),double(mode));
...
if strcmpi(opt.mask_type, 'and')
   mask_err = mask_Iepi_t .* mask_Ist_rigid;
else
   mask_err = mask_Iepi_t + mask_Ist_rigid - (mask_Iepi_t .* mask_Ist_rigid);
end

diff_img = Ist_rigid - Iepi_t_inv_msked;
X_error = sum((diff_img(:).^2).*mask_err(:))/numel(diff_img);

if opt.debug
   display_volume([Ist_rigid; Iepi_t_inv_msked; Iepi_t; Iepi], [0 1]);
   display_volume([mask_err; mask_Ist_rigid; mask_Iepi_t], [0 1]);
   overlay_volumes([(diff_img.^2 .*(mask_err*10)); Ist_rigid; Iepi_t_inv_msked; Iepi_t], [mask_err; mask_err; mask_err; mask_err]/3);
   overlay_volumes([(diff_img.^2 .*(mask_err*10)); Ist_rigid; Iepi_t_inv; Iepi_t], [mask_err; mask_Ist_rigid; mask_Iepi_t; mask_Iepi_t]/3);
end


% gradient
if nargout > 1   
   X_error_grad = gradient_analytical_intensity_correct(spacing, O_grid, rigid_param, rigid_scl, size(O_grid), ...
      res, Iepi_t, Iepi_w, Iepi_t_inv, Ist_rigid, crct_grd, diff_img, mask_err, mask_Ist_rigid, mask_Iepi_t, opt);   
end
...
function X_grad = gradient_analytical_intensity_correct(spacing, O_grid, rigid_param, rigid_scl, O_grid_size,...
   res, Iepi_t, Iepi_w, Iepi_t_inv, Ist_rigid, crct_grd, diff_img, mask_err, mask_Ist_rigid, mask_Iepi_t, opt)
...
d_Fmv_t1 = -2 .* double(mask_err) .* diff_img.* ...
           (Iepi_t_inv .* g_mask_Iepi_t - double(mask_Iepi_t) .* g_intsty_map .* (1+crct_grd) .* g_Iepi_w);
        
d_Fmv_t2 = -2 .* double(mask_err) .* diff_img.* (-1 .* double(mask_Iepi_t) .* g_intsty_map .* Iepi_w);

if strcmpi(opt.mask_type, 'and')   
   d_Fmv_t3 =  (diff_img.^2) .* double(mask_Ist_rigid).* g_mask_Iepi_t;
else
   d_Fmv_t3 = (diff_img.^2) .* (1-double(mask_Ist_rigid)) .* g_mask_Iepi_t;
end
d_Fmv = d_Fmv_t1 + d_Fmv_t3;

O_grad = zeros(O_grid_size(1:3));
d_P = zeros(numel(mask_err)+1, 1); % for fake zero voxel
for zi=1:4
   for zj=1:4
      for zk=1:4
         ...
         [bspline_prod, dbspline_prod] = ...
            bspline_repeated_endpoint_deformation_3d_double_only_x_gradient(O_temp, size(mask_err), spacing, opt.nthreads);
         dbspline_prod = dbspline_prod./spacing(1);
         
         d_Prod = (d_Fmv .* bspline_prod) + (d_Fmv_t2 .* dbspline_prod);
         d_P(1:(end-1)) = d_Prod(:);
