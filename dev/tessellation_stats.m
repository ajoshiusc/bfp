...
%<autobegin> ---------------------- 27-Jun-2005 10:45:58 -----------------------
...
%   c = cross(a,b);
%
% At Check-in: $Author: mosher $  $Revision: 20 $  $Date: 2006-02-10 21:03:10 -0500 (Fri, 10 Feb 2006) $
%
...
%                 parameters. 
% JCM 14-Jul-2017 Cleaning up some verbose printing
% ----------------------------- Script History ---------------------------------
...
if VERBOSE,
   ...
   fprintf('\nGeneral Poincaré Formula Check:\n')
   fprintf(...
      'There are %.0f faces, %.0f vertices, %.0f edges,\n',numTri,length(VertNdx),numEdges);
   fprintf('such that the Poincaré Formula is %.0f + %.0f - %.0f = %.0f\n',...
   ...
   if EulerCharacteristic == 2,
      ...
      disp(' Not a surface of genus 0 (not a closed surface)');
   end
   
   disp(' ')
   fprintf('Poincaré Formula Check for Triangles.\nIn a closed surface, then each edge has two faces, and each face has three edges.\n')
   fprintf('So 2 * edges = 3 * faces, and 2*faces + 2*vertices - 2*edges = 2 * Euler Characteristic = 2*vertices - faces.\n')
   fprintf('Then for a Euler Characteristic of 2 (closed spherical surface), in a truly closed surface of triangles, \n')
   fprintf('then the number of faces can be found as: number of faces  ==   2 * (the number of vertices - 2).\n')
   fprintf('\nHere, with %.0f triangles comprising %.0f vertices,\n',numTri, length(VertNdx))
   if numTri == 2*(length(VertNdx) - 2),
       fprintf('the numbers %.0f == %.0f here suggest:\n\n CLOSED SURFACE\n\n',numTri,2*(length(VertNdx) - 2))
   else
       fprintf('the numbers %.0f ~= %.0f here suggest:\n\n OPEN SURFACE\n\n',numTri,2*(length(VertNdx) - 2))
   end
