...
sublist = dir('/deneb_disk/parkinsons_fmri/neurocon/sub*');

for j=1:length(sublist)

    subid= sublist(j).name;

    t1=['/deneb_disk/parkinsons_fmri/neurocon/',subid,'/anat/',subid,'_T1w.nii.gz'];
    fmri=['/deneb_disk/parkinsons_fmri/neurocon/',subid,'/func/',subid,'_task-resting_run-1_bold.nii.gz'];

    %fmri='/home/ajoshi/Downloads/BFP_issues/ACTL005/ACTL005.BOLD.resting.nii.gz';
    studydir='/deneb_disk/parkinsons_fmri/neurocon_bfp_out';
