...
function [X_error, X_error_grad] = bspline_analytic_grad_SSD_EPI( ...
                                                    X, Osize1, O_grid, spacing, rigid_scl, ...
                                                    Iepi, Ist, res, mask_Iepi, mask_Ist, opt)                                                 
...
diff_img = Ist_rigid - Iepi_t;
X_error = sum((diff_img(:).^2).*mask_err(:))/numel(diff_img);

if opt.debug
   display_volume([Ist_rigid; Iepi_t; Iepi], [0 1]);
   display_volume([mask_err; mask_Ist_rigid; mask_Iepi_t], [0 1]);
   overlay_volumes([(diff_img.^2 .*(mask_err*10)); Ist_rigid; Iepi_t], [mask_err; mask_err; mask_err]/3);
   overlay_volumes([(diff_img.^2 .*(mask_err*10)); Ist_rigid; Iepi_t], [mask_err; mask_Ist_rigid; mask_Iepi_t]/3);
end
...
if nargout > 1
   X_error_grad = gradient_analytical_intensity_correct(spacing, O_grid, rigid_param, rigid_scl, size(O_grid), ...
      Iepi_w, Ist_rigid, crct_grd, diff_img, mask_err, mask_Ist_rigid, mask_Iepi_t, opt);
end
...
temp = -2 .* double(mask_err) .* diff_img;
d_Fmv_t1 = temp .* (1+crct_grd) .* g_Iepi_w;        
d_Fmv_t2 = temp .* Iepi_w;
...
[xd, yd, zd] = ndgrid(0:size(Ist_rigid,1)-1, 0:size(Ist_rigid,2)-1, 0:size(Ist_rigid,3)-1);
X_origin = size(Ist_rigid)/2;
S = eye(4);
