...
if fid < 0,
   ...
   if fread(fid,1,'int32') == 348
      hdr = read_header_load_nii_hdr(fid);
      fclose(fid);
   else
      fclose(fid);
...
%---------------------------------------------------------------------
function [ dsr ] = read_header_load_nii_hdr(fid)
...
dsr.hk   = header_key_load_nii_hdr(fid);
dsr.dime = image_dimension_load_nii_hdr(fid);
dsr.hist = data_history_load_nii_hdr(fid);
...
%---------------------------------------------------------------------
function [ hk ] = header_key_load_nii_hdr(fid)
...
%---------------------------------------------------------------------
function [ dime ] = image_dimension_load_nii_hdr(fid)
...
%---------------------------------------------------------------------
function [ hist ] = data_history_load_nii_hdr(fid)
