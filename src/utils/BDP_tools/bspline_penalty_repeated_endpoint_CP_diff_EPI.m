...
function [O_penalty, O_grad] = bspline_penalty_repeated_endpoint_CP_diff_EPI(O_grid1, spacing, res, penalty_wt)
...
c_knots = c_knots/spacing_mm(1); % Normalize w.r.t. to knot-spacing 
clear O_grid1

c_diff_x = diff(c_knots, 1, 1);
c_diff_y = diff(c_knots, 1, 2);
c_diff_z = diff(c_knots, 1, 3);
clear c_knots

cp_vol = prod(2*spacing_mm); % volume of region of affected by one cp-difference; Normalize w.r.t. number of points? 
...
if nargout==2
   ...
   O_penalty = O_penalty/cp_vol; % scale by influence volume of CP to account for difference in spacing in multi-resolution
   O_grad = O_grad/cp_vol;
else
