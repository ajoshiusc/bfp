...
def vis_grayord_sigpval(pval, rval, surf_name, out_dir, smooth_iter):
    plsurf, prsurf = label_surf(pval, [0,0.05], smooth_iter, 'jet_r')
    # If p value above .05 then make the surface grey
    plsurf.vColor[plsurf.attributes >= 0.05, :] = .5
    prsurf.vColor[prsurf.attributes >= 0.05, :] = .5
...
def label_surf(pval, colorbar_lim, smooth_iter, colormap):
    lsurf = readdfs('supp_data/bci32kleft.dfs')
    rsurf = readdfs('supp_data/bci32kright.dfs')
...
def vis_save_surf(lsurf, rsurf, out_dir, surf_name):
    # if label is zero, black out surface, attribute should be nan
    num_vert = lsurf.vertices.shape[0]
    lab = spio.loadmat('supp_data/USCBrain_grayordinate_labels.mat')
    labs = lab['labels'].squeeze(); labs = sp.float64(labs)
